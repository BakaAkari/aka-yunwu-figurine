...
3. 支持3种核心风格的图像转换，每种风格都有独立的命令和硬编码提示词
4. 提示词直接硬编码在代码中，无需用户配置，确保效果一致性
5. 轮询机制使用bot对象主动发送，避免session失效
6. 多张图片发送时添加延时，避免刷屏
7. 简化的任务状态管理（只用Map跟踪任务ID）
8. 无需中间件监听，代码更简洁
9. 内置超时机制，无需手动管理定时器
...
# 自定义提示词（通用命令）
图像处理 [图片] -p "make it more colorful"
图像处理 [图片] -p "enhance quality" -n 3
